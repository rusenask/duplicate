...
func testTools(code int, body string) (*httptest.Server, *hv.DBClient) {
	...
	cache := hv.Cache{DS: TestDB, RequestsBucket: bucket}

	cfg := hv.InitSettings()
	counter := hv.NewModeCounter()
	// preparing client
	dbClient := &hv.DBClient{
...
func setup() {
	...
	db := hv.GetDB(testingDatabaseName)
	TestDB = db
}
